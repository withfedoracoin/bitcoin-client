...
describe Fedoracoin::API do
  subject { Fedoracoin::API.new(:user => $user, :pass => $pass) }
  ...
  it "should accept host, port, ssl options" do
    req = Fedoracoin::API.new(:user => $user, :pass => $pass, :host => 'example.com', :port => 1234, :ssl => true)
